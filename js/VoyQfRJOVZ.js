...
for (let i = 0; i < jsData.table.rows.length; i++) {
    ...
    for (let j = 0; j < jsData.table.cols.length; j++) {
        if(!!jsData.table.cols[j].label){
            ...
            var value =(typeof(jsData.table.rows[i].c[j].v)!='number' && jsData.table.rows[i].c[j].f) ? jsData.table.rows[i].c[j].f : jsData.table.rows[i].c[j].v;
            }else{
            var value ="";
            }
...
for (var i = 0; i < SSCONN.primevideoDB.length; i++) {
	...
	columnaDEAcciones.innerHTML=
	`<a href="#u${i+1}" type="button" class="btn btn-warning" id="u${i+1}"><i class="fas fa-pencil-alt"></i></a>
	<a href="#u${i+1}" type="button" class="btn btn-danger" id="d${i+1}"><i class="far fa-trash-alt"></i></a>
	`
		...
		var columna =document.createElement("th");
			...
			switch(header){
		  		case "Dias_Vigentes":
		  			...
		  			else if(objetoFila[header]>=26  &&
		  			objetoFila[header]<30
		  			){columna.style.color="yellow";}
		  			else if(objetoFila[header]>=30  &&
...
function AccionBorrar(e){
	...
	let id = Number(this.parentNode.parentNode.getElementsByTagName("th")[5].textContent)

	let nombreCliente = this.parentNode.parentNode.getElementsByTagName("th")[2].textContent;
	...
	var	resultado = window.confirm("Seguro que quieres borrar a "+nombreCliente+ ", con id: "+id+"?" )
...
function EventoEditar(enlaceEditar, indiceBtn) {
	...
	function AccionEditar(e){
                if(document.querySelector("#revisar-popup")){document.querySelector("#revisar-popup").outerHTML=""};
		...
		var formulario = document.createElement("form");
                 var idFila = Number(enlaceEditar.parentNode.parentNode.getElementsByTagName("th")[5].textContent);
		...
		for (header in objetoFilaAEditar){
			if(header){
				...
				if(header == "Dias_Vigentes"){
					...
					input.value = "= INDIRECT(CONCAT(\"C\";ROW())) + "+ objetoFilaAEditar.sumar;
					input.setAttribute("readonly","");
			...
			btnAtras.addEventListener("click",()=>{location.reload()})
 ...
 function eventoActualizar (e){
	...
	var inputActualizar=document.querySelectorAll("#formulario_editar input");
			...
			if(header==inputActualizar[i].name){
				 ...
				 if(header==='Whatsapp'){
					...
					objetoFilaAEditar[header]=inputActualizar[i].value
                                ...
                                if(header==='sumar'){
                                        objetoFilaAEditar['sumar']=document.querySelector("[name=Fecha_Finalizacion]").value.split("+")[1]
                                }
...
function accionDeBuscar(){
	...
	for (var i = 0; i < SSCONN.primevideoDB.length; i++) {
			...
			var test = regExp.test(SSCONN.primevideoDB[i].Whatsapp) || regExp.test(SSCONN.primevideoDB[i].Nombre)
...
btnBorrar.addEventListener("click",()=>{ inputBuscar.value=""; function_ID()})
...
primordiales.sort(function(a,b){return b-a})
...
revisadosOrdenados = revisadosOrdenados.reduce((acc,item)=>{
    ...
    if(turno=="no" && turno ==0){
          $mensageDialog.innerHTML=revisadosOrdenados[revisadosOrdenados.length-1].message;
          buttonDeslizar.href="#u"+revisadosOrdenados[revisadosOrdenados.length-1].id;
...
function copiarContacto(){
        ...
        var textContact = document.querySelector("#"+document.querySelector("#button-deslizar").href.split("#")[1]).parentElement.parentNode.children[12].textContent;
        ...
        if(!(textContact.length==10)){
                ...
                document.execCommand("copy");
                textArea.outerHTML="";
                var nombre = document.querySelector("#"+document.querySelector("#button-deslizar").href.split("#")[1]).parentElement.parentNode.children[2].textContent;
                var aChat=document.getElementById("ir-a-chat");
                aChat.href="whatsapp://send/?text=Hola "+nombre+"&phone=57"+textContact;
                aChat.click()
        }
...
document.getElementById("copy-contact").addEventListener("click", copiarContacto);
